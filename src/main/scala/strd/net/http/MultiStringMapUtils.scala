...
trait MultiStringMapUtils {
  implicit class MultiStringMapWrapper(map: MultiStringMap) {
    ...
    def getFirstStrict(key: String) = map.get(key).flatMap(_.headOption).getOrElse{ throw new IllegalArgumentException(s"key $key is not found") }
    ...
    def parameter(key:String) = map.get(key).flatMap(_.headOption).getOrElse{
      throw BadRequest(HttpResponseStatus.BAD_REQUEST,  msg = Some("expected parameter:" + key))
    }
