...
object HttpStream {
  def factory(scheduler: ScheduledExecutorService, timeout: Int, metrics: HttpMetrics) = (ctx: ChannelHandlerContext, req: HttpReq) => {
    if (req.keepAlive) {
      new QueueHttpStream(ctx, timeout, scheduler, metrics)
    } else {
      new SingleHttpStream(ctx, timeout, scheduler, metrics)
    }
  }
}

class SingleHttpStream(ctx: ChannelHandlerContext,
                       timeout: Int,
                       scheduler: ScheduledExecutorService,
                       val metrics : HttpMetrics) extends HttpStream {
    ...
    request match {
      case Some(r) =>
        if (r.id != reqId || !scheduledWrite.compareAndSet(false, true)) {
          metrics.unorderedReq()
        } else {
          metrics.time(System.currentTimeMillis() - r.started)
          writeResponse(resp)
        }
      case None =>
        metrics.unorderedReq()
  ...
  def writeResponse(resp: HttpResp) {
    metrics.response(resp.status.code())
...
class QueueHttpStream(ctx: ChannelHandlerContext, timeout: Int, scheduler: ScheduledExecutorService, val metrics: HttpMetrics) extends HttpStream {
...
//        log.warn("Can not submit response")
  ...
  def stop() {
    ...
    metrics.keepAlive(counter.get, now - streamStarted)
  ...
  final def purge0(reqIter :  java.util.Iterator[StreamReqId], respIter : java.util.Iterator[StreamResponse], needFlush : Boolean ) : Boolean = {
    ...
    if (resp.isDefined && resp.get.reqId == req.id) {
      ...
      metrics.time(System.currentTimeMillis() - req.ts)
