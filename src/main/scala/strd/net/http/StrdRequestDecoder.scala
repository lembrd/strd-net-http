...
class StrdRequestDecoder extends MessageToMessageDecoder[FullHttpRequest]{
  ...
  override def decode(ctx: ChannelHandlerContext, msg: FullHttpRequest, out: util.List[AnyRef])  {
    ...
    val req = HttpReqImpl(
      ...
      uri = msg.uri(),
      headers = MultiStringMap(msgHeaders),
      content = if (buf.readableBytes() > 0) buf.nioBuffer().asReadOnlyBuffer() else ByteBuffer.allocate(0),
      keepAlive = HttpUtil.isKeepAlive(msg),
      ip = getRemoteAddr(ctx.channel(), msgHeaders))
...
class StrdResponseEncoder(serverName : Option[String]) extends MessageToMessageEncoder[HttpResp] {
  ...
  def encode(ctx: ChannelHandlerContext, msg: HttpResp, out: util.List[AnyRef]) = {
    try {
      ...
      serverName.foreach(x => headers.set(SERVER, x))
